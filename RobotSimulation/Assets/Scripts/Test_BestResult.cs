...
///调用时直接使用All函数
/// </summary>
public class Test_BestResult : MonoBehaviour
{
    /// <summary>
    ...
    public static double[] ALL(double[] IKInput, double[] oldtheta)
    {
        ...
        for (int i = 0; i < oldtheta.Length; i++)
        {
            ...
            oldtheta[i] = oldtheta[i] / 180D * Math.PI;

        }
        //Matrix T06FK = FK(oldtheta, PValue, ref AllTMatrix);
    ...
    {
        Debug.Log("============================计算逆解============================");
        Debug.Log("所输入的xyzwpr为：" + IKInput[0] + " " + IKInput[1] + " " + IKInput[2] + " "
                                         + IKInput[3] + " " + IKInput[4] + " " + IKInput[5] + " ");
        //0、装解的集合    
        ...
        //BestResult2(Theta, PValue, T06);

        Debug.Log("最优解关节角度：" + TheatBest[0] + " " + TheatBest[1] + " " + TheatBest[2] + " "
                                 + TheatBest[3] + " " + TheatBest[4] + " " + TheatBest[5] + " ");
    ...
    /// <summary>
    /// 最优解2：在求解的集合中找最合适的一组解
    /// </summary>
    /// <param name="theta">解的集合</param>
    /// <returns>返回最合适的解</returns>
    private static void BestResult2(double[,] theta, double[] PValue, Matrix T06)
    {
        Debug.Log("============================计算最优============================");
        ...
        for (int i = 0; i < theta.GetLength(0); i++)
        {
            ...
            //提取每一组解
            double[] resultTheta = { theta[i, 0], theta[i, 1], theta[i, 2], theta[i, 3], theta[i, 4], theta[i, 5] };
            Debug.Log("输入的角度为" + theta[i, 0] + " " + theta[i, 1] + " " + theta[i, 2] + " " + theta[i, 3] + " " + theta[i, 4] + " " + theta[i, 5]);

            //由角度值换成弧度值
            for (int j = 0; j < resultTheta.Length; j++)
            {
                resultTheta[j] = resultTheta[j] / 180D * Math.PI;
            }

            tempT06 = FK(resultTheta, PValue, ref AllTMatrix);

            if (Math.Abs(tempT06.Detail[0, 3] - T06.Detail[0, 3]) < 1 && Math.Abs(tempT06.Detail[1, 3] - T06.Detail[1, 3]) < 1 && Math.Abs(tempT06.Detail[2, 3] - T06.Detail[2, 3]) < 1)
            {
                indexResult.Add(i);
            }
        }
        ...
        {
            Debug.Log("筛选后的剩余解index为：" + indexResult[i]);
        }
        Debug.Log("============================结束最优============================");
    }
